    ...
    DefaultSecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http.authorizeHttpRequests(auth ->
                        auth
                                ...
                                .requestMatchers(HttpMethod.POST, "api/users/convert-to-organizer").hasRole("ADMIN")
                                .requestMatchers(HttpMethod.GET, "/api/users/id").hasAnyRole("USER", "ADMIN", "ORGANIZADOR")
                                ...
                                .requestMatchers(HttpMethod.GET, "/api/organization/user").hasRole("ORGANIZADOR")
                                .requestMatchers(HttpMethod.GET, "/api/roles").hasRole("ADMIN")
                                .requestMatchers(HttpMethod.GET, "/api/events").hasAnyRole("USER", "ADMIN", "ORGANIZADOR")
                                .requestMatchers(HttpMethod.GET, "/api/events/active").hasRole("USER")
                                .requestMatchers(HttpMethod.POST, "/api/events").hasAnyRole("ORGANIZADOR", "USER")
                                .requestMatchers(HttpMethod.PUT, "/api/events/{id}").hasRole("ORGANIZADOR")
                                .requestMatchers(HttpMethod.GET, "/api/events/available/{userId}").hasRole("USER")
                                .requestMatchers(HttpMethod.GET, "/api/events/registered/{userId}").hasRole("USER")
                                .requestMatchers(HttpMethod.DELETE, "/api/events/{eventId}/volunteers/{volunteerId}").hasAnyRole("ORGANIZADOR", "USER")
                                .requestMatchers(HttpMethod.POST, "/api/events/{eventId}/volunteers/{userId}\"").hasAnyRole("USER", "ORGANIZADOR")
                                .requestMatchers(HttpMethod.DELETE, "/api/events/{id}").hasRole("ORGANIZADOR")
